...
def list_lst(item):
    ...
    bloque = scrapertools.find_single_match(data, '>Categories<(.*?)</select>')

    matches = re.compile('<option class="level-0".*?value="(.*?)">(.*?)</option>').findall(bloque)
...
def list_all(item):
    ...
    matches = re.compile('<div class="recent-item">(.*?)</p>').findall(data)

    if not matches:
        if not '<div id="categories-3"' in data: matches = re.compile('<article(.*?)</article>').findall(data)
    ...
    for match in matches:
        ...
        title = title.replace('Capitulo', '[COLOR goldenrod]Cap.[/COLOR]').replace('Capítulo', '[COLOR goldenrod]Cap.[/COLOR]')
    ...
    if itemlist:
        ...
        if next_page:
            if '/page/' in next_page:
                thumb = os.path.join(config.get_runtime_path(), 'resources', 'media', 'channels', 'thumb', 'ennovelasws.jpg')
...
def episodios(item):
    ...
    if item.cat:
        ...
        if not matches: matches = re.compile('<div class="related-item">(.*?)</p>', re.DOTALL).findall(data)
        ...
        if config.get_setting('channels_charges', default=True): item.perpage = sum_parts
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('EnNovelasWs', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('EnNovelasWs', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts > 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos?'):
                    platformtools.dialog_notification('EnNovelasWs', '[COLOR cyan]Cargando elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('EnNovelasWs', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75
