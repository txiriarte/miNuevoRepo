...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_lilatorrent_proxies', default=''):
...
def do_downloadpage(url, post=None, headers=None):
    hay_proxies = False
    if config.get_setting('channel_lilatorrent_proxies', default=''): hay_proxies = True

    if not url.startswith(host):
        data = httptools.downloadpage(url, post=post, headers=headers).data
    else:
        if hay_proxies:
            data = httptools.downloadpage_proxy('lilatorrent', url, post=post, headers=headers).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers).data
...
def acciones(item):
    ...
    itemlist.append(item.clone( channel='submnuctext', action='_test_webs', title='Test Web del canal [COLOR yellow][B] ' + host + '[/B][/COLOR]',
                                from_channel='lilatorrent', folder=False, text_color='chartreuse' ))
    ...
    if item.search_type == "movie":
        ...
        if matches:
            ...
            for url, thumb in matches:
                ...
                if "1 Temporada" in title: SerieName = title.split("1 Temporada")[0]
                elif "2 Temporada" in title: SerieName = title.split("2 Temporada")[0]
                elif "3 Temporada" in title: SerieName = title.split("3 Temporada")[0]
                elif "4 Temporada" in title: SerieName = title.split("4 Temporada")[0]
                elif "5 Temporada" in title: SerieName = title.split("5 Temporada")[0]
    ...
    if item.url.endswith('.torrent'):
        if config.get_setting('proxies', item.channel, default=''):
            if PY3:
                ...
                data = requeststools.read(item.url, 'lilatorrent')
            else:
                data = do_downloadpage(item.url)
