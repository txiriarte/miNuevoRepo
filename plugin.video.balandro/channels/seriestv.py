...
def item_configurar_proxies(item):
    ...
    if config.get_setting('channel_seriestv_proxies', default=''):
...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    ant_hosts = ['https://www.veronline.co/']
    ...
    hay_proxies = False
    if config.get_setting('channel_seriestv_proxies', default=''): hay_proxies = True
    ...
    if not url.startswith(host):
        ...
        if hay_proxies:
            data = httptools.downloadpage_proxy('seriestv', url, post=post, headers=headers, raise_weberror=raise_weberror, timeout=timeout).data
        else:
            data = httptools.downloadpage(url, post=post, headers=headers, raise_weberror=raise_weberror, timeout=timeout).data

        if not data:
            if not 'recherche?q=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
...
def acciones(item):
    ...
    itemlist.append(item.clone( channel='submnuctext', action='_test_webs', title='Test Web del canal [COLOR yellow][B] ' + host + '[/B][/COLOR]',
                                from_channel='seriestv', folder=False, text_color='chartreuse' ))
...
def list_all(item):
    ...
    bloque = scrapertools.find_single_match(data, '<span>Veronline.pro</span>(.*?)>mas vistas<')

    if not bloque: bloque = scrapertools.find_single_match(data, '<span>veronline.pro</span>(.*?)>mas vistas<')
...
def episodios(item):
    ...
    if item.page == 0 and item.perpage == 50:
        ...
        if config.get_setting('channels_charges', default=True): item.perpage = sum_parts
            ...
            if sum_parts > 50:
                platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando Todos los elementos[/COLOR]')
            ...
            if sum_parts >= 1000:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]500[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando 500 elementos[/COLOR]')
                    item.perpage = 500

            elif sum_parts >= 500:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando 250 elementos[/COLOR]')
                    item.perpage = 250

            elif sum_parts >= 250:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]125[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando 125 elementos[/COLOR]')
                    item.perpage = 125

            elif sum_parts >= 125:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]75[/B][/COLOR] elementos ?'):
                    platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando 75 elementos[/COLOR]')
                    item.perpage = 75

            elif sum_parts > 50:
                if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos [COLOR cyan][B]Todos[/B][/COLOR] de una sola vez ?'):
                    platformtools.dialog_notification('SeriesTv', '[COLOR cyan]Cargando ' + str(sum_parts) + ' elementos[/COLOR]')
                    item.perpage = sum_parts
    ...
    for option in options:
        ...
        if d_url:
            ...
            try:
                ...
                if hay_proxies:
                    url = httptools.downloadpage_proxy('seriestv', d_url, follow_redirects=False, timeout=timeout).headers['location']
                else:
