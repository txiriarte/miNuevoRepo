...
def mainlist_animes(item):
    ...
    itemlist.append(item.clone( title = 'Últimos animes', action = 'list_all', url = host, group = 'last_anim', search_type = 'tvshow', text_color = 'moccasin' ))
...
def list_all(item):
    ...
    elif item.group == 'last_anim': bloque = scrapertools.find_single_match(data, '>Últimos Animes<(.*?)</section>')
    else: bloque = data
